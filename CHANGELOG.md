...
- Added `contentBitrate` attribute to video events. This uses `segmentationBitrate` if available, falling back to `streamBitrate`.
- Added `adBitrate` attribute to ad events. This extracts the ad bitrate using multiple methods and standardizes the value to bps (bits per second).
